/* matching_statistics_stats - Computes the matching statistics from BWT and Thresholds
...
*/
/*!
   \file matching_statistics_stats.cpp
   \brief matching_statistics_stats.cpp Computes the matching statistics from BWT and Thresholds.
...
public:
  ...
  void matching_statistics(kseq_t *read, FILE* out)
  {
    ...
    fwrite(&max_l, sizeof(size_t), 1,out);
  }
...
void parseArgs(int argc, char *const argv[], Args &arg)
{
  ...
  std::string usage("usage: " + std::string(argv[0]) + " infile [-s store] [-m memo] [-c csv] [-p patterns] [-t threads] [-l len] [-q shaped_slp]\n\n" +
                    "Computes the statistics of the matching statistics.\n" +
...
void dispatcher(Args &args)
{
  ...
  for (size_t i = 0; i < args.th; ++i)
  {
    ...
    size_t max_l = 0;
    while (!feof(in_fd) and fread(&max_l, sizeof(size_t), 1, in_fd) > 0)
    {
      if(max_l >= 75) mem75++;
      if(max_l >= 50) mem50++;
      if(max_l >= 25) mem25++;
  ...
  verbose("Number of reads: ", n_seq);
  verbose("Reads with minimum length 25: ", mem25);
  verbose("Reads with minimum length 50: ", mem50);
  verbose("Reads with minimum length 75: ", mem75);
