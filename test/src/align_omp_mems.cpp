...
public:
  ...
  bool align(kseq_t *read, FILE* out, std::size_t& mem_25, std::size_t& mem_50, std::size_t& mem_75)
  {
    size_t mem_pos = 0;
    size_t mem_len = 0;
    size_t mem_idx = 0;
    ...
    //------------------------------
    if (mem_len >= 25) { mem_25++; }
    if (mem_len >= 50) { mem_50++; }
    if (mem_len >= 75) { mem_75++; }
    //------------------------------
...
// {
  ...
  for(size_t i = 1; i <= 64; ++i)
  {
    ...
    std::size_t mem_25 = 0, mem_50 = 0, mem_75 = 0;
    ...
    {
      if(aligner.align(seq,sam_fd, mem_25, mem_50, mem_75))
        n_aligned_reads_p ++;
      n_reads_p++;
    ...
    std::string out_csv_path = file_path + ".csv";
    std::ofstream out_csv(out_csv_path);
    out_csv << "mem_gt_25,mem_gt_50,mem_gt_75\n";
    out_csv << mem_25 << ",";
    out_csv << mem_50 << ",";
    out_csv << mem_75 << "\n";
    out_csv.close();
